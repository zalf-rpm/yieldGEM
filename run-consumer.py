...
def run_consumer(server={"server": None, "port": None}):
    ...
    def process_message(msg):
        ...
        if msg["type"] == "finish":
            ...
            print("c: received work result ", process_message.received_env_count, " customId: ",
            ...
            #         writer.writerow(["id", "kg N", "mm irrigation", "year", "sowing date", "harvest date", "yield", "AbBiom"])
            ...
            #             if len(r) == 0:
            #                 continue
            #             row = [cid["id"], cid["nfert"], cid["irrigation"]]
            ...
            #             row.append(r["AbBiom"])
            #             writer.writerow(row)
            # 使用 'w' 模式打开文件会在每次写入时覆盖整个文件，因此每次循环时，新的数据都会覆盖之前写入的数据。要避免这种情况，应该使用 'a' 模式（追加模式）打开文件，而不是 'w' 模式。
            with open(config["path_to_out_file"], 'a', newline='') as _:
                ...
                for data_ in msg.get("data", []):
                    ...
                    print(results)
                    row = [cid["id"]]
                    # row.append(results[0]["DOY"])
                    row.append(results[0]["Yield"])
                    writer.writerow(row)
